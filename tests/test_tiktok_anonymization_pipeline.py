...
class TikTokAnonymizationPipelineTest(unittest.TestCase):
    ...
    @patch("tiktok_anonymization_pipeline.generate_similarity_variant")
    ...
    def test_pipeline_runs_end_to_end_and_writes_result(
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            variant = root / "pipeline" / "jobs" / "7621519315451448596" / "variant" / "variant.mp4"
            ...
            def _variant_side_effect(_input, output_path, **_kwargs):
                target = Path(output_path)
                target.parent.mkdir(parents=True, exist_ok=True)
                target.write_bytes(b"variant")
                (target.parent / "variant-summary.json").write_text('{"ok": true}', encoding="utf-8")
                return {"ok": True}
            ...
            mock_variant.side_effect = _variant_side_effect
            ...
            self.assertTrue(result["similarity_variant_applied"])
            self.assertTrue(Path(result["result_path"]).exists())
            self.assertEqual(mock_detect.call_args.args[0], variant)
            self.assertEqual(mock_overlay.call_args.args[0], variant)
            self.assertEqual(mock_validate.call_args.args[0], variant)
    ...
    @patch("tiktok_anonymization_pipeline.track_faces_in_detections")
    @patch("tiktok_anonymization_pipeline.detect_faces_in_video")
    @patch("tiktok_anonymization_pipeline.extract_reference_audio_from_tiktok")
    @patch("tiktok_anonymization_pipeline.generate_similarity_variant")
    @patch("tiktok_anonymization_pipeline.download_tiktok_video_job")
    def test_pipeline_uses_external_reference_audio_when_requested(
        self,
        mock_download,
        mock_variant,
        mock_reference_audio,
        mock_detect,
        mock_track,
        mock_overlay,
        mock_validate,
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            source = normalized.parent / "source.mp4"
            source.write_bytes(b"source")
            reference_audio = root / "downloads" / "reference-audio" / "audio123" / "reference_audio.m4a"
            ...
            mock_reference_audio.return_value.audio_path = str(reference_audio)
            ...
            result = run_tiktok_anonymization_pipeline(
                ...
                output_root=root / "pipeline",
                export_dir=export_root,
                reference_audio_url="https://www.tiktok.com/@milk/video/123",
            )

            self.assertTrue(result["used_external_reference_audio"])
            self.assertEqual(result["reference_audio_url"], "https://www.tiktok.com/@milk/video/123")
            self.assertEqual(mock_variant.call_args.kwargs["audio_input"], reference_audio)
            mock_reference_audio.assert_called_once()
